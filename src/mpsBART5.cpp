        ...
        for(int i = 0; i < t_nodes.size();i++){
                ...
                for(int j=0;j<data.d_var;j++){
                        ...
                        // if(t_nodes[i]->ancestors(j)>0){
                                ...
                                arma::mat aux_precision_inv = arma::inv(t_nodes[i]->B_t.slice(j)*t_nodes[i]->B.slice(j)+((data.tau_b(j))/data.tau)*data.diag);
        ...
        for(int j = 0; j < data.d_var;j++){
                ...
                for(int t = 0; t< all_trees.trees.size();t++){
                        ...
                        for(int i = 0; i< t_nodes.size(); i++ ){
                                ...
                                beta_sq_sum_total(j) = beta_sq_sum_total(j) + arma::as_scalar(t_nodes[i]->betas.col(j).t()*data.diag*t_nodes[i]->betas.col(j));
